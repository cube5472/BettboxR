...
func (t *Tuic) dial(ctx context.Context) (quicConn *quic.Conn, err error) {
        ...
        if err != nil {
                // Некоторые серверы (нарушая RFC 9001 §8.1) вообще не выбирают ALPN,
                // хотя клиент его предложил. Строгая проверка клиента обрывает
                // рукопожатие ("server did not select an ALPN protocol"). Повторяем
                // рукопожатие без ALPN — так же ведёт себя sing-box (NekoBox).
                if strings.Contains(err.Error(), "server did not select an ALPN protocol") {
                        noAlpnConfig := t.tlsConfig.Clone()
                        noAlpnConfig.NextProtos = nil
                        _, quicConn, err = common.DialQuic(ctx, t.addr, t.DialOptions(), t.dialer, noAlpnConfig, t.quicConfig, common.DialQuicOption{Early: t.option.ReduceRtt})
                }
                if err != nil {
                        return nil, err
                }
