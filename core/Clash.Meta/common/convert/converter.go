...
func ConvertsV2Ray(buf []byte) ([]map[string]any, error) {
        ...
        for _, raw := range rawLines {
                ...
                case "tuic":
                        ...
                        // Панели отдают tuic-ссылки с self-signed сертификатами
                        // (sni-маска + pinSHA256). Без skip-cert-verify такой узел
                        // не проходит TLS, остальные протоколы это поле мапят.
                        if query.Get("insecure") == "1" || query.Get("allow_insecure") == "1" ||
                                query.Get("allowInsecure") == "1" {
